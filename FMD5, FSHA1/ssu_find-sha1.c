...
void main(int argc, char* argv[]) {
	...
	//search
	search_fsha1(FILE_EXTENSION, minSize, maxSize, TARGET_DIRECTORY);
	//print
	if(printList()==-1) {
...
//				if((statbuf2.st_mode & S_IRUSR)==0 || (statbuf2.st_mode & S_IRGRP)==0 || (statbuf2.st_mode & S_IROTH)==0) continue;
				...
				if(S_ISREG(statbuf2.st_mode)) { //정규파일일 경우에만. 해시, 사이즈 구하기
					...
					strcpy(sample.hash, getSHA1Hash(pathbuf));
				}
...
char* getSHA1Hash(char* path) {
	...
	while(1) {
		...
		if(len<=0) break;
		SHA1_Update(&ctx, buf, (unsigned long)len);
	}
	SHA1_Final(&(md[0]), &ctx);
	
	sprintf(hash, "%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x", md[0], md[1], md[2], md[3], md[4], md[5], md[6], md[7], md[8], md[9], md[10], md[11], md[12], md[13], md[14], md[15], md[16], md[17], md[18], md[19]);
